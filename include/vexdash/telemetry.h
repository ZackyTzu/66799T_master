...
// RAISED 64 -> 96 (2026-08-21, F2-f). ChannelId is uint16_t, so the id space
// is nowhere near this; the cost is the fixed ChannelInfo table (~68 bytes
// each, +32 entries = ~2.2 KB of static RAM) and a longer link-up
// registration burst. The burst is safe to lengthen: every CHANNEL_DEF frame
// goes out through bounded_retry_write(), which flow-controls and gives up
// per FRAME (its own stall/total budget), so more frames means a longer
// registration, never a truncated one. kMaxSamplesPerFrame above is a
// SEPARATE and much tighter limit on how many put()s fit between two
// flushes -- raising this does NOT raise that, and that is the one that
// actually bites on a channel-heavy robot. dplib's own template and
// downstream team repos already shipped at 96; this brings lib-core (and the
// website's downloadable zip, packed from this header) back in step.
// 中文：上限從 64 提到 96（2026-08-21，F2-f）。ChannelId 是 uint16_t，編號空間
// 差得遠；代價是固定表變大（每筆約 68 bytes，多 32 筆約 2.2KB 靜態記憶體）與
// link-up 註冊 burst 變長。burst 變長是安全的：每一幀都走 bounded_retry_write
// 流控、而且預算是「每幀」各自計算，所以只會變慢、不會被截斷。注意上面的
// kMaxSamplesPerFrame 是另一個、而且緊得多的限制（每兩次 flush 之間能塞幾個
// put），提高這裡**不會**放寬那個。dplib 範本與下游學生 repo 早就是 96，這裡
// 把 lib-core（連帶網站下載 zip）補回同一格。
