...
class WatchRegistry {
 public:
  ...
  // RAISED 64 -> 96（2026-08-21，F2-f），與 lib-core 的 kMaxChannels／
  // kMaxConfigParams 同步調高——一台裝滿感測器/機構的機器人（每顆馬達
  // pos/rpm/temp/amp 全掛）很容易吃滿 64 格；dplib 範本與下游學生 repo早就是
  // 96，這裡補回同一格，網站下載 zip 也是從這顆 header 打包出來的。
  ...
  // 迴圈裡連續送出最多 kMaxWatches (96) 個 CHANNEL_DEF/CONFIG_SCHEMA 幀、中間完
 ...
 private:
  ...
  struct Entry {
    ...
    // ＝kMaxWatches(96) × 64 bytes ≒ 6KB 靜態儲存，與 name 那格同級。
